...
def run_sims():
	...
	# Also save subgroup membership and train indics if requested
	if task_settings["save_subgroup_membership"]:
		benchmark.region_membership_df.to_csv(SAVEPREFIX + ".membership.{}.tsv".format(task_settings["task_id"]), sep="\t", index=False)
		benchmark.train_indic_df.to_csv(SAVEPREFIX + ".train_indic.{}.tsv".format(task_settings["task_id"]), sep="\t", index=False)
